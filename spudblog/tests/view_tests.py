...
class BaseTestCase(TestCase):
    def setUp(self):
        ...
        self.blog1 = Blog.objects.create(author=self.user,
                                         title='Writing spudblog',
                                         background='plain-green')
        self.post1_1 = Post.objects.create(blog=self.blog1,
                                           title='Angularjs UI',
                                           content='Creating models and views')
        self.post1_2 = Post.objects.create(blog=self.blog1,
                                           title='Django server',
                                           content='Creating site and app')
        self.blog2 = Blog.objects.create(author=self.user,
                                         title='Deploying spudblog',
...
class ApiTest(BaseTestCase):
    ...
    def test_get_full_blog(self):
        ...
        self.assertEquals('Creating models and views',
                          content['posts'][0]['content'])
        self.assertEquals('Creating site and app',
    ...
    def test_blog_udate(self):
        ...
        response = self.client.put('/api/blog/%d' % self.blog2.id,
                                   content_type='application/json',
                                   data=json.dumps(changed_blog))
        self.assertEquals(response.status_code, 200)
        ...
        self.assertEquals('plain-blue',
                          updated_blog['background'])  # un-altered
...
class UIViewTest(BaseTestCase):
    ...
    def test_my_blogs(self):
        ...
        self.assertEquals(response.status_code, 200)
        self.assertEquals('Writing spudblog',
                          response.context.get('blogs')[0].title)
        self.assertEquals('Deploying spudblog',
                          response.context.get('blogs')[1].title)
